    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (token, method, endpoint) => {
          ...
          try {
            // Mock the actual request to prevent real API calls
            vi.spyOn(apiClient, method as any).mockResolvedValue({ data: {} });
        ...
        async (token) => {
          ...
          const requestSpy = vi.fn((config) => {
            if (config.headers && config.headers.Authorization) {
              ...
              const parts = config.headers.Authorization.split(' ');
