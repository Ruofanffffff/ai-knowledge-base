...
const SchemaKG: React.FC = () => {
  ...
  const fetchKGData = async () => {
    ...
    try {
      ...
      // 提取实体类型
      const types = Array.from(new Set(entitiesData.entities.map((e: Entity) => e.type))) as string[];
  ...
  const filterData = () => {
    ...
    // 过滤关系
    const filteredRelations = graphData.relations.filter(r => {
  ...
  const drawGraph = () => {
    ...
    node.on('mouseover', function(_event, d) {
      ...
      d3.select(this).select('circle')
        .transition()
        .duration(200)
        .attr('r', 25 + (entity.confidence * 20))
        .attr('stroke-width', 3);
      ...
      d3.select(this).select('circle')
        .transition()
        .duration(200)
        .attr('r', 20 + (entity.confidence * 20))
        .attr('stroke-width', 2);
    });
