...
const GLOBAL_CSS = `
  ...
  .auth-input               { caret-color: rgba(210,170,255,0.9); }
  .otp-input                { caret-color: transparent; text-align: center; }
  .otp-input::selection     { background: rgba(155,89,182,0.3); }
...
const BG_URL =
  'https://images.unsplash.com/photo-1771068048946-a0dfd494534a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzcHJpbmclMjBjaGVycnklMjBibG9zc29tJTIwc2FrdXJhJTIwYm9rZWglMjBzb2Z0JTIwbGlnaHR8ZW58MXx8fHwxNzcxODYyMTI3fDA&ixlib=rb-4.1.0&q=80&w=1080';

const BOKEH = [
  { w: 180, h: 200, x: 6,  y: 8,  c: 'rgba(255,180,196,0.28)', dur: 9,  d: 0  },
  { w: 260, h: 240, x: 78, y: 4,  c: 'rgba(255,210,225,0.2)',  dur: 12, d: -4 },
  { w: 130, h: 130, x: 48, y: 58, c: 'rgba(220,185,255,0.24)', dur: 8,  d: -6 },
  { w: 210, h: 190, x: 88, y: 42, c: 'rgba(255,195,215,0.18)', dur: 14, d: -2 },
  { w: 100, h: 100, x: 18, y: 76, c: 'rgba(200,220,255,0.26)', dur: 7,  d: -5 },
  { w: 190, h: 170, x: 62, y: 22, c: 'rgba(255,230,200,0.2)',  dur: 15, d: -9 },
  { w: 140, h: 150, x: 35, y: 88, c: 'rgba(255,180,210,0.22)', dur: 10, d: -3 },
  { w: 120, h: 110, x: 94, y: 78, c: 'rgba(195,215,255,0.2)',  dur: 11, d: -7 },
];
...
function SpringPetals() {
  ...
  useEffect(() => {
    ...
    const mk = (): P => ({
      ...
      sz: Math.random()*10+3, rot: Math.random()*Math.PI*2, rv: (Math.random()-0.5)*0.022,
      op: Math.random()*0.52+0.18, hue: Math.random()*35-8,
      st: Math.random()*Math.PI*2, ss: Math.random()*0.009+0.004,
    });
    const petals: P[] = Array.from({ length: 65 }, mk);
    const draw = (p: P) => {
      ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.rot); ctx.globalAlpha = p.op;
      const h = 338 + p.hue;
      ctx.fillStyle = `hsl(${h},82%,87%)`;
      ctx.beginPath();
      ctx.moveTo(0, -p.sz);
      ...
      ctx.fill();
      ctx.globalAlpha = p.op*0.35; ctx.strokeStyle = `hsl(${h+15},70%,94%)`;
      ctx.lineWidth = p.sz*0.12; ctx.lineCap = 'round';
      ctx.beginPath(); ctx.moveTo(0, -p.sz*0.85);
      ctx.quadraticCurveTo(p.sz*0.18, -p.sz*0.1, 0, p.sz*0.18); ctx.stroke();
      ctx.restore();
    };
    let raf: number;
    const tick = () => {
      ...
      for (const p of petals) {
        p.st += p.ss; p.x += p.vx + Math.sin(p.st)*0.7; p.y += p.vy; p.rot += p.rv;
  ...
  return (
    <div>
      <div style={{
        ...
        background: focused ? 'rgba(255,255,255,0.16)' : 'rgba(255,255,255,0.09)',
        border: `1.5px solid ${focused ? 'rgba(195,130,255,0.7)' : error ? 'rgba(255,110,100,0.5)' : 'rgba(255,255,255,0.2)'}`,
        boxShadow: focused
          ? 'inset 0 1px 0 rgba(255,255,255,0.22),0 0 0 3px rgba(150,80,255,0.15),0 4px 24px rgba(170,80,240,0.12)'
          : 'inset 0 1px 0 rgba(255,255,255,0.1)',
        transition: 'all 0.22s ease',
      }}>
        <Icon size={15} style={{ color: focused ? 'rgba(210,160,255,1)' : 'rgba(255,255,255,0.42)', flexShrink: 0, transition: 'color 0.2s' }} />
        <input type={type ?? 'text'} inputMode={inputMode} placeholder={placeholder} value={value}
          ...
          style={{ color: 'rgba(255,255,255,0.93)', fontSize: '14px', minWidth: 0 }} />
        {rightSlot}
      </div>
      <AnimatePresence>
        {error && (
...
function EyeBtn({ show, toggle }: { show: boolean; toggle: () => void }) {
  return (
    <motion.button whileTap={{ scale: 0.85 }} onClick={toggle} type="button" style={{ flexShrink: 0 }}>
      {show ? <EyeOff size={14} style={{ color: 'rgba(255,255,255,0.38)' }} />
             : <Eye    size={14} style={{ color: 'rgba(255,255,255,0.38)' }} />}
    </motion.button>
  );
}
  ...
  return (
    <div className="flex items-center justify-center gap-2 mb-5">
      {Array.from({ length: total }, (_, i) => (
        <motion.div key={i} animate={{ width: i === current ? 22 : 8, background: i < current ? '#22C55E' : i === current ? '#C084FC' : 'rgba(255,255,255,0.22)' }}
          transition={{ duration: 0.32, ease: [0.16,1,0.3,1] }}
          style={{ height: 8, borderRadius: 4, boxShadow: i === current ? '0 0 8px rgba(192,132,252,0.7)' : 'none' }} />
      ))}
    </div>
  );
}
          ...
          <motion.div key={i} animate={d ? { scale: [1,1.22,0.95,1] } : { scale: 1 }}
            ...
            <input
              ...
              style={{
                ...
                background: d ? 'rgba(192,132,252,0.18)' : 'rgba(255,255,255,0.09)',
                border: `2px solid ${d ? 'rgba(192,132,252,0.8)' : error ? 'rgba(255,110,100,0.5)' : 'rgba(255,255,255,0.22)'}`,
                color: 'rgba(255,255,255,0.95)',
                boxShadow: d ? '0 0 12px rgba(192,132,252,0.35)' : 'none',
  ...
  return (
    <div className="mt-2 flex items-center gap-2">
      <div className="flex gap-1 flex-1">
        {[1,2,3,4].map(i => (
          <motion.div key={i} animate={{ background: i <= s ? STRENGTH_COLOR[s] : 'rgba(255,255,255,0.15)', scaleX: i <= s ? 1 : 0.5 }}
      ...
      <motion.span key={s} initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }}
        style={{ fontSize: '11px', color: STRENGTH_COLOR[s] || 'rgba(255,255,255,0.4)', minWidth: 20, textAlign: 'right' }}>
        {STRENGTH_LABEL[s]}
      </motion.span>
    </div>
  );
    ...
    <div className="relative flex flex-col items-center px-5 pb-7 pt-2">
      ...
      <div className="relative mb-5">
        <motion.div animate={{ scale: [1, 1.5, 1], opacity: [0.4, 0, 0] }}
          ...
          style={{ background: 'rgba(192,132,252,0.4)' }} />
      ...
      <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
        ...
        style={{ color: 'rgba(255,255,255,0.97)', fontSize: '22px', fontWeight: 900, letterSpacing: '-0.02em', marginBottom: 6 }}>
        注册成功！🎉
      </motion.p>
      <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.6, duration: 0.5 }}
        style={{ color: 'rgba(255,255,255,0.55)', fontSize: '13.5px', marginBottom: 24, textAlign: 'center' }}>
      ...
      {/* Progress bar */}
      <div className="w-full" style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 4, height: 4, overflow: 'hidden' }}>
      ...
      <p className="mt-2" style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px' }}>{countdown}s 后进入 Hi Brain</p>
    </div>
  );
}
...
export function Auth() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let res;
      if (method === 'phone') {
        // Current backend /login expects password. 
        // If user is trying to login with phone code, we might need to handle it.
        // For now, assuming password login is primary or we use phone as username.
        // But the UI asks for CODE for phone login.
        // Backend doesn't seem to have verify-code-login endpoint in the snippet I saw.
        // I'll try to use the code as password or just mock it if it fails?
        // No, let's try to send phone and code.
        res = await authService.login({ phone: form.phone, code: form.code });
      } else {
      ...
      if (res.success) {
        ...
        window.dispatchEvent(new Event('auth_login')); // Notify app
        navigate('/home');
      } else {
        setErrors(prev => ({ ...prev, [method === 'phone' ? 'code' : 'password']: res.message || '登录失败' }));
        shake();
      }
    } catch (err: any) {
      console.error(err);
      setErrors(prev => ({ ...prev, [method === 'phone' ? 'code' : 'password']: err.response?.data?.error || '登录失败' }));
      shake();
    } finally {
      setLoading(false);
    }
    ...
    return (
      <motion.button whileTap={{ scale: 0.9 }} onClick={sendCode} disabled={countdown > 0} type="button"
        style={{ flexShrink:0, whiteSpace:'nowrap', fontSize:'12px', fontWeight:600,
          color: countdown > 0 ? 'rgba(255,255,255,0.45)' : 'rgba(200,160,255,1)',
          display:'flex', alignItems:'center', gap:4, padding:'2px 0' }}>
        {countdown > 0 ? (
          <>
            <svg width="20" height="20" style={{ transform: 'rotate(-90deg)', flexShrink:0 }}>
              <circle cx="10" cy="10" r={r} fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="2.2" />
              <circle cx="10" cy="10" r={r} fill="none" stroke="rgba(190,150,255,0.85)" strokeWidth="2.2"
  ...
  const handlePhoneProfile = async () => {
    ...
    try {
      const res = await authService.register({
        ...
        password: regPwd,
        nickname: regNick,
        // code: otp.join('') // If backend needs verification code
      });
      
      if (res.success) {
         // Auto login after register? Or just go to success screen.
         // Success screen handles navigation.
         goForward(setPhoneStep);
      } else {
         setRegPwdErr(res.message || '注册失败');
      }
    } catch (err: any) {
       setRegPwdErr(err.response?.data?.error || '注册失败');
    } finally {
      setRegLoading(false);
    }
  };
  ...
  const handleEmailProfile = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setRegNickErr(res.message || '注册失败');
      }
    } catch (err: any) {
      setRegNickErr(err.response?.data?.error || '注册失败');
    } finally {
      setRegLoading(false);
    }
  };
  ...
  const RegMethodSelect = () => (
    <div className="px-5 pb-5">
      <motion.p initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}
        style={{ color:'rgba(255,255,255,0.55)', fontSize:'13px', marginBottom:16, textAlign:'center' }}>
        选择注册方式
      </motion.p>
      ...
      <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.4 }}
        className="text-center mt-4" style={{ color:'rgba(255,255,255,0.28)', fontSize:'11px' }}>
        注册即代表同意《用户协议》及《隐私政策》
      </motion.p>
    </div>
  );
  ...
  const PhoneStep0 = () => (
    <div className="space-y-4">
      <div className="text-center mb-2">
        ...
        <p style={{ color:'rgba(255,255,255,0.9)', fontSize:'17px', fontWeight:800 }}>输入手机号</p>
        <p style={{ color:'rgba(255,255,255,0.42)', fontSize:'12.5px', marginTop:3 }}>我们将向您发送6位验证码</p>
      </div>
    ...
    return (
      <div className="space-y-5">
        <div className="text-center mb-1">
          ...
          <p style={{ color:'rgba(255,255,255,0.9)', fontSize:'17px', fontWeight:800 }}>输入验证码</p>
          <p style={{ color:'rgba(255,255,255,0.42)', fontSize:'12.5px', marginTop:3 }}>
            已发送至 <span style={{ color:'rgba(192,132,252,0.9)', fontWeight:600 }}>{regPhone.slice(0,3)}****{regPhone.slice(-4)}</span>
          </p>
        </div>
        ...
        <div className="flex justify-center">
          <motion.button whileTap={{ scale:0.9 }} onClick={() => { if (regCd === 0) startRegCd(); }}
            disabled={regCd > 0}
            style={{ fontSize:'12.5px', color: regCd > 0 ? 'rgba(255,255,255,0.35)' : 'rgba(192,132,252,0.9)',
              display:'flex', alignItems:'center', gap:4, fontWeight:500 }}>
            {regCd > 0 ? (
              <>
                <svg width="18" height="18" style={{ transform:'rotate(-90deg)' }}>
                  <circle cx="9" cy="9" r={r} fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="2" />
                  <circle cx="9" cy="9" r={r} fill="none" stroke="rgba(192,132,252,0.8)" strokeWidth="2"
  ...
  ──────────────────────────────────────────────────*/
  return (
    <div className="fixed inset-0 overflow-hidden select-none" style={{ background:'#0e0520' }}>
      ...
      <motion.div className="absolute inset-[-8%]"
        ...
        <img src={BG_URL} className="w-full h-full object-cover"
          style={{ filter:'brightness(0.78) saturate(1.4) contrast(1.05)' }} alt="" draggable={false} />
      </motion.div>

      {/* ── L1: Grades ── */}
      <div className="absolute inset-0 z-[1]" style={{ background:'linear-gradient(170deg,rgba(255,185,210,0.22) 0%,rgba(160,100,230,0.28) 45%,rgba(80,110,230,0.18) 80%,rgba(240,140,180,0.16) 100%)' }} />
      <div className="absolute inset-0 z-[1]" style={{ background:'radial-gradient(ellipse 90% 85% at 50% 38%,transparent 0%,rgba(8,3,24,0.62) 100%)' }} />
      <div className="absolute bottom-0 left-0 right-0 h-1/3 z-[1]" style={{ background:'linear-gradient(0deg,rgba(20,8,45,0.75) 0%,transparent 100%)' }} />

      {/* ── L2: Bokeh ── */}
      <div className="absolute inset-0 z-[2] pointer-events-none overflow-hidden">
        {BOKEH.map((b, i) => (
          <motion.div key={i} className="absolute rounded-full"
            style={{ width:b.w, height:b.h, left:`${b.x}%`, top:`${b.y}%`, background:b.c, filter:`blur(${Math.max(b.w,b.h)*0.38}px)`, transform:'translate(-50%,-50%)' }}
            animate={{ y:[0,-28,16,-22,0], x:[0,16,-12,10,0], scale:[1,1.14,0.93,1.1,1] }}
      ...
      <div className="absolute inset-0 z-[4] pointer-events-none overflow-hidden">
        {[{ left:'28%',rot:7,dur:6,delay:0 },{ left:'52%',rot:-5,dur:9,delay:-3 },{ left:'70%',rot:3,dur:7,delay:-6 }].map((s, i) => (
          <motion.div key={i} className="absolute top-0"
            style={{ left:s.left, width:2, height:'75%', background:'linear-gradient(180deg,rgba(255,240,200,0.2) 0%,transparent 100%)', filter:'blur(10px)', transform:`rotate(${s.rot}deg)`, transformOrigin:'top center' }}
            animate={{ opacity:[0.3,0.85,0.2,0.65,0.3], scaleX:[1,2.2,0.7,1.8,1] }}
      ...
      <motion.p initial={{ opacity:0 }} animate={{ opacity:[0,0.45,0.28,0.45] }}
        ...
        style={{ color:'rgba(255,230,240,0.6)', fontSize:'11px', fontWeight:300, letterSpacing:'0.4em' }}>
        春风拂面 · 万物生长
      </motion.p>
      ...
      <div className="absolute inset-0 z-[10] flex items-center justify-center px-5 overflow-y-auto py-8">
        <motion.div initial={{ opacity:0, y:44, scale:0.92 }} animate={{ opacity:1, y:0, scale:1 }}
          ...
          style={{
            background:'rgba(255,250,255,0.13)',
            backdropFilter:'blur(36px)', WebkitBackdropFilter:'blur(36px)',
            borderRadius:30,
            border:'1px solid rgba(255,255,255,0.38)',
            boxShadow:'0 28px 72px rgba(100,40,160,0.3),0 4px 16px rgba(255,180,210,0.18),inset 0 1px 0 rgba(255,255,255,0.55)',
            overflow:'hidden',
          }}>
          ...
          {!(tab === 'register' && ((regMode === 'phone' && phoneStep === 3) || (regMode === 'email' && emailStep === 3))) && (
            <motion.div initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }}
              ...
              <div className="w-14 h-14 rounded-[20px] flex items-center justify-center mb-3 relative"
                style={{ background:'linear-gradient(135deg,rgba(255,140,175,0.88),rgba(130,70,220,0.88))', boxShadow:'0 10px 32px rgba(130,70,220,0.45),inset 0 1px 0 rgba(255,255,255,0.4)' }}>
          ...
          {!(tab === 'register' && ((regMode === 'phone' && phoneStep === 3) || (regMode === 'email' && emailStep === 3))) && (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ duration:1.4, delay:1.1 }}
              className="mx-5 mb-4 p-1 rounded-2xl flex relative" style={{ background:'rgba(255,255,255,0.1)' }}>
              <motion.div className="absolute top-1 bottom-1 rounded-xl"
                animate={{ left: tab === 'login' ? '4px' : 'calc(50%)', right: tab === 'login' ? 'calc(50%)' : '4px' }}
                transition={{ duration:0.3, ease:[0.16,1,0.3,1] }}
                style={{ background:'rgba(255,255,255,0.22)', backdropFilter:'blur(8px)', boxShadow:'0 2px 8px rgba(0,0,0,0.12)' }} />
                ...
                <div className="flex gap-2 mb-4">
                  {(['phone','email'] as const).map(m => (
                    <motion.button key={m} whileTap={{ scale:0.95 }} onClick={() => { setMethod(m); setErrors({}); }}
                      ...
                      style={{ background: method===m ? 'rgba(170,110,255,0.28)' : 'rgba(255,255,255,0.08)',
                        border:`1px solid ${method===m ? 'rgba(190,130,255,0.65)' : 'rgba(255,255,255,0.15)'}`,
                        color: method===m ? 'rgba(215,175,255,0.98)' : 'rgba(255,255,255,0.42)',
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div key={method} initial={{ opacity:0, x: method==='phone' ? -14 : 14, y:4 }}
                      ...
                      <motion.button whileTap={{ scale:0.97 }} onClick={handleSubmit} disabled={loading || success}
                        className="w-full py-[14px] rounded-2xl flex items-center justify-center gap-2 relative overflow-hidden mt-1"
                        animate={{ background: success ? ['linear-gradient(135deg,#22C55E,#16A34A)'] : ['linear-gradient(135deg,rgba(255,120,165,0.92) 0%,rgba(140,70,225,0.92) 100%)'] }}
                        style={{ boxShadow: success ? '0 8px 28px rgba(34,197,94,0.45)' : '0 8px 28px rgba(140,70,225,0.45)' }}>
