...
const handleSubmit = async () => {
    ...
    try {
      const payload = method === 'phone' 
        ? { phone: form.phone, code: form.code } // Assuming backend supports phone+code login or similar
        : { email: form.email, password: form.password };
        
      // If using phone login, backend expects different payload probably. 
      // But looking at server.js, local login expects { username, email, phone, password }.
      // Let's assume email login for now as it matches the form.
      ...
      if (data.success) {
        ...
        else setErrors(e => ({ ...e, code: data.error || '登录失败' }));
        shake();
      }
    } catch (err: any) {
      console.error(err);
